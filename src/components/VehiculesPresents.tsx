...
export const VehiculesPresents: React.FC<VehiculesPresentsProps> = ({
  ...
  const [filterNuit, setFilterNuit] = useState<string>('all');
  ...
  const filtered = presents.filter((s) => {
    ...
    const matchNuit =
      filterNuit === 'all' ||
      (filterNuit === 'nuit' && s.reste_la_nuit) ||
      (filterNuit === 'jour' && !s.reste_la_nuit);

    return matchSearch && matchClient && matchRep && matchNuit;
  });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            value={filterNuit}
            onChange={(e) => setFilterNuit(e.target.value)}
            className="px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs font-semibold text-slate-700"
          >
            <option value="all">Suivi Nuit / Jour (Tous)</option>
            <option value="nuit">🌙 Resté la nuit</option>
              ...
              <div>
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <div className="mt-1">
                      {st.reste_la_nuit ? (
                        <span className="inline-flex items-center gap-1 text-[11px] font-bold text-purple-800 bg-purple-100 px-2 py-0.5 rounded-full border border-purple-200">
                          <Moon className="w-3 h-3" />
                          <span>Resté la nuit</span>
                        </span>
                      ) : (
